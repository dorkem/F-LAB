...
- 커넥션 하나가 트랜잭션 하나를 책임집니다.
- 그리고 `@Transactional`은 결국 이 try-catch 패턴을 직접 안 써도 되게 스프링이 프록시로 감싸서 대신 실행해주는 겁니다.

<br>

### 스레드가 동시에 돌면 뭐가 문제냐

- 서버에 요청이 두 개 동시에 들어오면 스레드 A, B가 각자 실행됩니다. 만약 커넥션을 싱글톤 객체의 필드에 저장하면 이렇게 됩니다.

```
스레드 A: conn = getConnection()   // 커넥션 획득
스레드 B: conn = getConnection()   // 같은 필드를 덮어씀
스레드 A: conn으로 쿼리 실행       // 이미 B가 덮어쓴 커넥션임
스레드 A: commit()                 // B의 트랜잭션까지 같이 커밋됨
```

- 트랜잭션이 뒤섞입니다.
- A가 롤백해야 하는데 B의 커밋이 먼저 터지는 식으로 데이터가 오염됩니다.
...
- 커넥션은 결국 객체입니다.
- 그 객체의 참조값을 **스레드 전용 사물함(스레드 로컬)** 에 넣어두는 겁니다.

```
스레드 A의 사물함 → conn_A 객체 주소 저장
스레드 B의 사물함 → conn_B 객체 주소 저장

레포지토리가 쿼리 실행할 때:
"내 스레드 사물함에서 커넥션 꺼내자" → 알아서 자기 것만 나옴
```

- 파라미터로 커넥션을 들고 다니는 게 아니라, 같은 스레드 안이면 어디서 꺼내도 항상 같은 커넥션이 나옵니다. 그래서 서비스 → 레포지토리 A → 레포지토리 B로 호출이 이어져도 트랜잭션이 유지됩니다.

<br>

### @Transactional의 실제 흐름

```
1. 커넥션 꺼내서 스레드 로컬에 저장
2. setAutoCommit(false) → 트랜잭션 시작
3. 내가 작성한 메서드 실행
      ↓
   레포지토리 호출할 때마다
   스레드 로컬에서 같은 커넥션 꺼내서 쿼리 실행
4. 정상 종료 → commit
5. 예외 발생 → rollback
6. 스레드 로컬에서 커넥션 제거 후 반납
```
...
- 서비스, 레포지토리 빈은 싱글톤입니다.
- 앱 전체에서 객체 하나를 모든 스레드가 공유합니다.
- 모든 스레드가 같은 필드를 건드리기 때문에 싱글톤 빈의 필드에 상태를 저장하면 안 됩니다. 
- 커넥션처럼 스레드마다 달라야 하는 상태는 반드시 스레드 로컬에 넣어야 합니다.
    - 프로토타입 빈도 마찬가지입니다.
    - 프로토타입은 꺼낼 때마다 새 객체를 만들어주기 때문에, 철수가 `CartHelper`를 꺼내면 철수만의 `items`, 영희가 꺼내면 영희만의 `items`를 가질 수 있습니다.
    - 그런데 이걸 싱글톤 필드에 `@Autowired`로 주입해버리면 앱 시작할 때 딱 한 번만 주입되고 고정됩니다.
    - 이후에는 철수 요청이 오든 영희 요청이 오든 **같은 `CartHelper` 객체 하나를 같이 씁니다.**
    - 결국 철수가 담은 상품과 영희가 담은 상품이 같은 `items` 리스트에 쌓여서 뒤섞입니다.

```java
@Service  // 싱글톤
public class OrderService {
    @Autowired
    private CartHelper cartHelper;  // ← 여기 박히는 순간 하나로 고정
    // 철수도 이 cartHelper, 영희도 이 cartHelper
}
```

- 프로토타입을 제대로 쓰려면 `@Autowired` 필드로 두는 게 아니라, 필요할 때마다 직접 꺼내와야 합니다.
- 하지만 실무에서는 그냥 메서드 안에서 `new CartHelper()`로 새로 만드는 게 더 간단하고 흔한 방식입니다.

<br>

### 스레드 로컬 정리를 반드시 해야 하는 이유

- 웹 서버는 스레드를 재사용합니다(스레드 풀). 요청이 끝났는데 스레드 로컬을 안 비우면, 다음 요청이 이전 요청의 커넥션이나 데이터를 그대로 물려받습니다.
- `@Transactional`이 끝날 때 커넥션을 반납하면서 스레드 로컬도 같이 정리하는 이유가 이겁니다.
...
- 싱글톤 빈들이 상태 없이 공유되면서도 스레드마다 독립적인 트랜잭션을 유지할 수 있는 게 스레드 로컬 덕분입니다.
- `@Transactional`은 그 스레드 로컬에 커넥션을 넣고 빼는 try-catch를 대신 써주는 어노테이션입니다.
