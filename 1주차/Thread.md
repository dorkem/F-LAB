## 프로세스와 쓰레드의 차이점
프로세스는 독립된 메모리 공간을 가지므로 데이터 교환을 위해 IPC가 필요하고, 스레드는 프로세스 내부에서 메모리를 공유하기 때문에 빠르지만 동기화 문제가 발생한다. 
CPU는 단일 코어에서는 Context Switching을 통해 동시성을 제공하고, 멀티 코어에서는 실제 병렬 실행이 가능하다. 
...
- 이때 스레드는 프로세스가 할당받은 `Heap·Static·Code` 영역을 공유하므로 여러 스레드가 동시에 접근하는 크리티컬 섹션에서 `Race Condition`이 발생할 수 있으며, 
- 이를 해결하기 위해 `synchronized`, `Lock`, `Atomic` 클래스, `concurrent 컬렉션`, `volatile` 등의 동기화 기법이 필요하다.
...
**프로그램이 실행되어 OS로부터 메모리를 할당받은 `독립된 메모리 공간`**
...
### 특징
- 독립된 메모리 공간을 가짐
    - Code / Data / Heap / Stack 전부 분리
- 프로세스끼리는 메모리를 공유하지 않기 때문에 프로세스 간 데이터 교환이 필요하면(IPC - Inter-Process Communication이 필요하다.)
- 무겁고 생성 비용이 큼
